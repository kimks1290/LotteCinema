...
<body>
	...
	<!-- 영화 리스트 SELECT 태그   -->
	<form action="${contextPath}/admin/modify2"
		enctype="multipart/form-data" method="get">
		<%--id=frm--%>
		<div class="movie-container">
			<label>영화제목:</label> <select id="movie" onChange="SelectRedirect();">
				<option>-- 영화선택 --</option>
				<option value="${movieList[0].price}"
					data-movno="${movieList[0].movNo}">${movieList[0].title}</option>
			</select>
		</div>
		...
		<div class="container">
			...
			<input type="text" name="theater" id="invisible"
				value="${info[0].theater}">
			<!-- 상영시간 -->
			<input type="text" name="time" id="invisible" value="${info[0].time}">
			<!-- 아 이 디  -->
