...
<body>
	...
	<!-- 영화 리스트 SELECT 태그   -->
	<form action="${contextPath}/admin/modify1"
		enctype="multipart/form-data" method="get">
		<%--id=frm--%>
		<div class="movie-container">
			<label>영화제목:</label> <select id="movie" onChange="SelectRedirect();">
				<option>-- 영화선택 --</option>
				<option value="${movieList[2].price}"
					data-movno="${movieList[2].movNo}">${movieList[2].title}</option>
			</select>
		</div>
		...
		<div class="container">
						...
						<c:choose>
							...
							<c:otherwise>
								<div class="seat ${i}" id="cntrl">${i.seats}<input
										id="invisible2" type="checkbox" name="seats"
										value="${i.seats }">
								</div>
							</c:otherwise>
						</c:choose>
						...
						<c:choose>
							...
							<c:otherwise>
								<div class="seat ${i}">${i.seats-8}<input type="checkbox"
										id="invisible2" name="seats" value="${i.seats }">
								</div>
							</c:otherwise>
						</c:choose>
					</c:if>
						...
						<c:choose>
							...
							<c:otherwise>
								<div class="seat ${i}">${i.seats-40}<input type="checkbox"
										id="invisible2" onclick="showHideBox()" name="seats"
										value="${i.seats }">
			...
			<!-- 예매번호  -->
			<div class="resbutton">
				<input type="submit" id="" value="예매하기"> <br>
				<br>
				<a href="/root/admin/logout1">LOGOUT</a>
			...
			<input type="text" name="theater" id="invisible"
				value="${info[2].theater}">
			<!-- 상영시간 -->
			<input type="text" name="time" id="invisible" value="${info[2].time}">
			<!-- 아 이 디  -->
